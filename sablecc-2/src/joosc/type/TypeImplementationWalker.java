 ...
 *
 * Second pass of type-checking module. 
 ...
 * and binds selected constructors and methods to invoking sites.
 *
 * History:
 *   23 May 2000 - fixed bug in outANotExp (correct type not int but bool)
...
public class TypeImplementationWalker extends AbstractTypeWalker {
	...
	/* MANDATORY entry point */
 	public static void walk(List theProgram, 
		...
		voidType = new AVoidType(new TVoid());
	...
	private boolean isApplicableSignature(AFormalList formals,
	                                      AArgumentList args) {
		Iterator iterF,iterA;
		
		// one list empty but not both => false
		if (formals == null || args == null)
			return (formals == null && args == null);
			
		iterF = formals.getFormal().iterator();
		iterA = args.getExp().iterator();
	...
	private boolean isMaxSpecificSignature(AFormalList formals,
	...
	private boolean isMoreSpecificSignature(AFormalList f1, 
	                                        AFormalList f2) {
		...
		iter1 = f1.getFormal().iterator();
		iter2 = f2.getFormal().iterator();
	...
	private void checkArgumentsAgainstFormals(AFormalList f, 
	                                          AArgumentList a,
	                                          int line) {
		Iterator iterF;
		Iterator iterA;
		...
		iterF = f.getFormal().iterator();
		iterA = a.getExp().iterator();
	...
	public void inAClass(AClass node) {
		curClass = node;
		checkUniqueConstructors(node.getConstructor());
	}
	...
	public void inAExternClass(AExternClass node) {
		curClass = node;
		checkUniqueConstructors(node.getExternConstructor());
	}
	...
	public void outAReturnStm(AReturnStm node) {
		...
		int line = node.getReturn().getLine();
		PType returnValueType;
		
		if ((returnType instanceof AVoidType) && (returnValue != null)) {
			MyError.error(filename,"return value not allowed",line);
		}
		if (!(returnType instanceof AVoidType) && (returnValue == null)) {
			MyError.error(filename,"return value expected",line);
		}
		if (!(returnType instanceof AVoidType) && (returnValue != null)) {
			returnValueType = typeOfNode(returnValue);
			if (!assignCompatible(returnType,returnValueType)) {
				MyError.error(filename,"illegal type of expression",line);
			}
		}
	}
	...
	public void outASuperconsStm(ASuperconsStm node) {
		...
		Node cons = selectConstructor(
		               ...
		               (AArgumentList)node.getArgumentList(),
		               node.getSuper().getLine());
	...
	public void outAIfStm(AIfStm node) {
		checkBoolean(typeOfNode(node.getExp()),node.getLPar().getLine());
	}
	...
	public void outAAssignStmExp(AAssignStmExp node) {
		typeTree.put(node,typeTree.get(node.getAssignment()));
	}
	public void outACallStmExp(ACallStmExp node) {
		typeTree.put(node,typeTree.get(node.getMethodinvocation()));
	}
	public void outANewStmExp(ANewStmExp node) {
		typeTree.put(node,typeTree.get(node.getClassinstancecreation()));
	}
	...
	public void outAOrExp(AOrExp node) {
		int line = node.getOr().getLine();
		
		coerceToStringMap.put(node,Boolean.FALSE);
		checkBoolean(typeOfNode(node.getLeft()),line);
		checkBoolean(typeOfNode(node.getRight()),line);
		typeTree.put(node,booleanType);
	}
	public void outAAndExp(AAndExp node) {
		int line = node.getAnd().getLine();
	...
	public void outAInstanceofExp(AInstanceofExp node) {
		...
		int line = node.getInstanceof().getLine();
	...
	public void outAUminusExp(AUminusExp node) {
		coerceToStringMap.put(node,Boolean.FALSE);
		checkInt(typeOfNode(node.getExp()),node.getMinus().getLine());
		typeTree.put(node,intType);
	}
	...
	public void outANotExp(ANotExp node) {
		coerceToStringMap.put(node,Boolean.FALSE);
		checkBoolean(typeOfNode(node.getExp()),node.getNot().getLine());
		typeTree.put(node,booleanType);
	}
	
	public void outACastExp(ACastExp node) {
		int line = node.getRPar().getLine();
	...
	public void outACasttocharExp(ACasttocharExp node) {
		coerceToStringMap.put(node,Boolean.FALSE);
		checkInt(typeOfNode(node.getExp()),node.getRPar().getLine());
	...
	public void outASuperReceiver(ASuperReceiver node) {
		int line = node.getSuper().getLine();
