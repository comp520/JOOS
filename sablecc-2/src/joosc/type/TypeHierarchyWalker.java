 ...
 * TypeHierarchyWalker
 *
 * First pass of type-checking module. 
 * Statically checks the class hierarchy, and in particular:
 * - inheritance validity
 * - field hiding validity
 * - method overriding validity
 *
 * History:
 *   23 May 2000 - corrected bug in inAnyMethod (wrong method node used)
 *                 feature-complete
 *   21 May 2000 - updated to take advantage of astToGenericMap
 *   17 May 2000 - created
 */
public class TypeHierarchyWalker extends AbstractTypeWalker {
	...
	public void inAModMethod(AModMethod node) {
		inAnyMethod(node.getIdentifier(),node.getFormalList(),
	...
	public void inAnyMethod(Token id, PFormalList formals, 
	                        PReturntype returntype,  Node node) {
		String name = id.getText();
		int line = id.getLine();
		// overriden method formals and return type
		PFormalList parFormals;
		PReturntype parReturntype;
		...
		if (s != null) {
			...
			if (s.kind() != methodSymK) {
				...
				if (gMethod.getModifier() == GenericMethod.FINAL) {
					...
					if (! equalFormals((AFormalList)formals,gMethod.getFormals()) ||
