...
/*
 * CodeGenerator - generates bytecode IR for the input program
 *
 * History:
 *    4 Jun 2000 - bug in caseACastcharExp fixed
 *    2 Jun 2000 - thorough code review (5 bugs fixed); AExpStm bug fixed
 *   31 May 2000 - feature-complete
 *   29 May 2000 - created
 */
...
public class CodeGenerator extends DepthFirstAdapter 
	...
	private String makeFormalsCode(AFormalList formals) {
		if (formals == null) return ""; // for main
		Iterator iter = formals.getFormal().iterator();
	...
	private String makeSignature(AFormalList f, PReturntype rt) {
		return "(" + makeFormalsCode(f) + ")" + makeReturntypeCode(rt);
	}
	
	private String makeConstructorSignature(AFormalList formals) {
	...
	public void outAConstructor(AConstructor node) {
		outAnyConstructor((AFormalList)node.getFormalList(),node);
	}
	public void outAExternConstructor(AExternConstructor node) {
		outAnyConstructor((AFormalList)node.getFormalList(),node);
	}
	// worker method
	private void outAnyConstructor(AFormalList formals, Node node) {
	...
	public void outAModMethod(AModMethod node) {
		outAnyMethod(node.getReturntype(),(AFormalList)node.getFormalList(),node);
	}
	...
	public void outAMainMethod(AMainMethod node) {
		outAnyMethod(new AVoidReturntype(new TVoid()),null,node);
	}
	...
	// worker method
	private void outAnyMethod(PReturntype rt, AFormalList f, Node node) {
	...
	public void caseAExpStm(AExpStm node) {
		...
		if (! (typeAnnotations.get(node.getStmExp()) instanceof AVoidType))
	...
	/* classinstancecreation production */
	...
	public void caseAClassinstancecreation(AClassinstancecreation node) {
		...
		if (node.getArgumentList() != null)
			node.getArgumentList().apply(this);
		chainInstr(new InvokenonvirtualInstr(
		                  makeConstructorCode(theClass,cons)));
	}
	...
	public void caseAMethodinvocation(AMethodinvocation node) {
		...
		if (receiver instanceof AObjectReceiver) {
			...
			chainInstr(new InvokevirtualInstr(
			              makeMethodCode(theClass,method)));
		}
