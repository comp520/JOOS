...
public class CodeEmitter extends DepthFirstAdapter {
	...
	public void caseAClass(AClass node) {
		...
		try {
			...
			out.println();
			for (iter = node.getField().iterator(); iter.hasNext();)
				((PField)iter.next()).apply(this);
			if (! node.getField().isEmpty())
				out.println();
			for (iter = node.getConstructor().iterator(); iter.hasNext();)
				((PConstructor)iter.next()).apply(this);
			for (iter = node.getMethod().iterator(); iter.hasNext();)
